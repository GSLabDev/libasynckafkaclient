...
inline void WriteInt8(Int8 value, std::stringstream& os)
{
  os.write( reinterpret_cast<char*>(&value), sizeof(Int8) );
}

inline void WriteInt16(Int16 value, std::stringstream& os)
{
  value = htons(value);
  os.write( reinterpret_cast<char*>(&value), sizeof(Int16) );
}

inline void WriteInt32(Int32 value, std::stringstream& os)
{
  value = htonl(value);
  os.write( reinterpret_cast<char*>(&value), sizeof(Int32) );
}

inline void WriteInt64(Int64 value, std::stringstream& os)
{
  value = host_to_be_64(value);
  os.write( reinterpret_cast<char*>(&value), sizeof(Int64) );
}

inline void WriteString(const String& value, std::stringstream& os)
{
  WriteInt16(static_cast<Int16>(value.size()), os);
  os.write( value.c_str(), value.size() );
}

inline void WriteBytes(const Bytes& value, std::stringstream& os)
{
  if (!value)
  {
    WriteInt32(-1, os);
...
inline void WriteMessage(const Message& value, std::stringstream& os)
{
  using boost::asio::buffer_cast;

  // Write everything (except crc) to an intermediate buffer
  std::stringstream intermediate_os;
  ...
  crc.process_bytes(intermediate_os.str().data(), intermediate_os.str().length());

  // Write to the real stream
  WriteInt32(crc.checksum(), os);
  os.write(intermediate_os.str().c_str(), intermediate_os.str().length());
}

inline void WriteMessageSet(const MessageSet& value, std::stringstream& os)
{
